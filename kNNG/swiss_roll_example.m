...
X = [tt.*cos(tt); height; tt.*sin(tt)]; % matrix of points, each col = 1 data pt
...
[dknn_3,Hknn_3,Lavgknn_3,Lstdknn_3]=knn_graph_estim_2(X,kneighbors,gamma,1,10,samp);       % (M,N)=(1,10)
...
% generate data
n = 2000; d = 10; samp = (n-10):(n-1); kneighbors=4; gamma=1;
X = GenerateManifoldData(1,d,n);

% algorithm
[dknn_1,Hknn_1,Lavgknn_1,Lstdknn_1]=knn_graph_estim_1(X,kneighbors,gamma,1,10,samp);       % (M,N)=(1,10)
disp(['Intrinsic dimension estimate = ',num2str(round(dknn_1))])                         % note that dknn_1 is rounded to the nearest integer
rng(10);

[dknn_2,Hknn_2,Lavgknn_2,Lstdknn_2]=knn_graph_estim_1(X,kneighbors,gamma,10,1,samp);       % (M,N)=(10,1)
disp(['Intrinsic dimension estimate = ',num2str(round(dknn_2))])
rng(10);

% Dumb implementation of k-NN algorithm. Much slower in low (extrinsic) dimensional spaces (like this example)
% but faster for high (extrinsic) dimensional spaces (like images)
[dknn_3,Hknn_3,Lavgknn_3,Lstdknn_3]=knn_graph_estim_2(X,kneighbors,gamma,1,10,samp);       % (M,N)=(1,10)
disp(['Intrinsic dimension estimate = ',num2str(round(dknn_3))])
rng(10);