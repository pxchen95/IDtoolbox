function [Y, R, E] = IsoMap(X, n_fcn, n_size, options)
...
%
% Input:
%    X      = DxN matrix, 1 column = 1 data point
...
%    Estimate ID as the elbow on the graph
...
D = pdist2(X',X'); % N x N matrix of distances (where N is the number of data points)
N = size(D,1); 
if ~(N==size(D,2))
...
elseif nargin < 4
     options = struct('dims',1:8,'overlay',1,'comp',1,'display',1,'verbose',1); 
end
