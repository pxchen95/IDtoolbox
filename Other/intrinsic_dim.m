...
%   
% INPUTS: 
%        X      = DxN matrix, 1 col = 1 data pt
%        method = 'CorrDim' (correlation dimension)
%               = 'NearNbDim' (nearest neighbor dimension)
%               = 'GMST' (geodesic minimum spanning tree)
%               = 'PackingNumbers' (data packing numbers)
%               = 'EigValue' (PCA)
%               = 'MLE' (maximum likelihood estimator, default method)
% 
...
%                 disp(l); disp(no_dims);
        ...
        case 'EigValue'
            ...
            % Evaluate residuals
            no_dims = 0;
            while no_dims < size(X, 2) && lambda(no_dims + 1) > 0.025
                no_dims = no_dims + 1;
            end
